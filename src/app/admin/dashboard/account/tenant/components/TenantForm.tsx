...
const tenantSettingsSchema = z.object({
  ...
  sessionTimeout: z
    .number()
    .min(
      FORM_VALIDATION_RULES.sessionTimeout.min,
      VALIDATION_MESSAGES.sessionTimeout.min
    )
    .max(
      FORM_VALIDATION_RULES.sessionTimeout.max,
      VALIDATION_MESSAGES.sessionTimeout.max
    )
});
...
const tenantFormSchema = z.object({
  name: z
    .string()
    .min(
      FORM_VALIDATION_RULES.name.minLength,
      VALIDATION_MESSAGES.name.minLength
    )
    .max(
      FORM_VALIDATION_RULES.name.maxLength,
      VALIDATION_MESSAGES.name.maxLength
    )
    .regex(
      FORM_VALIDATION_RULES.name.pattern!,
      VALIDATION_MESSAGES.name.pattern
    ),
  code: z
    .string()
    .min(
      FORM_VALIDATION_RULES.code.minLength,
      VALIDATION_MESSAGES.code.minLength
    )
    .max(
      FORM_VALIDATION_RULES.code.maxLength,
      VALIDATION_MESSAGES.code.maxLength
    )
    .regex(
      FORM_VALIDATION_RULES.code.pattern,
      VALIDATION_MESSAGES.code.pattern
    ),
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-6'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>配置租户的功能限制和默认设置</CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
              {/* 最大用户数 */}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>新注册用户的默认角色</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
