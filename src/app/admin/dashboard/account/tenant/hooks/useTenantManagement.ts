...
import { isTenantListData, isTenant, isTenantStats } from '../types';
...
export function useTenantManagement(): UseTenantManagementReturn {
  ...
  const fetchTenants = useCallback(async (filters: TenantFilters) => {
    try {
      ...
      if (response.success && response.data) {
        // fetchTenants 返回的是分页数据
        if (isTenantListData(response.data)) {
  ...
  const createTenant = useCallback(
    async (data: TenantFormData): Promise<boolean> => {
      try {
        ...
        if (response.success && response.data) {
          // createTenant 返回的是单个 Tenant 对象
          const tenant = isTenant(response.data)
            ? response.data
            : isTenantListData(response.data)
    ...
    async (
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          // updateTenantStatus 返回的是单个 Tenant 对象
          const tenant = isTenant(response.data)
            ? response.data
            : isTenantListData(response.data)
              ? response.data.data[0]
          ...
          if (tenant && isTenant(tenant)) {
            setTenants((prev) =>
              prev.map((t) =>
                t.id === id ? { ...t, status: tenant.status } : t
              )
            );
          }
  ...
  const fetchStatistics = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        // 获取统计数据
        const stats = isTenantStats(response.data) ? response.data : null;
        ...
        const statsData: TenantStatisticsData = {
          overview: {
            ...
            inactive: stats.inactiveTenants || 0,
            suspended: stats.suspendedTenants || 0,
            activeRate:
          ...
          users: {
            ...
            active: Math.round((stats.totalUsers || 0) * 0.8) // 估算值
          },
          engagement: {
            recentActive: Math.round((stats.totalUsers || 0) * 0.7),
            activeRate: 70,
            avgActivity: 50
          },
          growth: {
