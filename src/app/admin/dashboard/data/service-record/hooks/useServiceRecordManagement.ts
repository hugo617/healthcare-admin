...
import {
  ...
  ServiceRecordStatistics,
  PaginationInfo
} from '../types';
import { DEFAULT_PAGINATION, MESSAGES } from '../constants';
import { ServiceRecordAPI } from '@/service/api/service-record';

/**
 * 服务记录管理业务逻辑 Hook
 */
export function useServiceRecordManagement() {
  const [records, setRecords] = useState<ServiceRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [pagination, setPagination] =
    useState<PaginationInfo>(DEFAULT_PAGINATION);
  const [statistics, setStatistics] = useState<
    ServiceRecordStatistics | undefined
  >();
  ...
  const fetchRecords = useCallback(async (filters: any) => {
    try {
      ...
      const res = await ServiceRecordAPI.getServiceRecords(filters);

      if (res.success && res.data) {
        setRecords(res.data);
        setPagination(res.pagination || DEFAULT_PAGINATION);
      } else {
        toast.error(res.message || '获取服务记录失败');
        setRecords([]);
      }
    } catch (error) {
      console.error('获取服务记录失败:', error);
      toast.error('获取服务记录失败');
      setRecords([]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchStatistics = useCallback(async (params?: any) => {
    try {
      const res = await ServiceRecordAPI.getServiceRecordStatistics(params);
  ...
  const createRecord = useCallback(
    async (data: ServiceRecordFormData): Promise<boolean> => {
      try {
        const res = await ServiceRecordAPI.createServiceRecord(data);
  ...
  const updateRecord = useCallback(
    async (
      id: string,
      data: Partial<ServiceRecordFormData>
    ): Promise<boolean> => {
      try {
        const res = await ServiceRecordAPI.updateServiceRecord(id, data);
  ...
  const deleteRecord = useCallback(async (id: string): Promise<boolean> => {
    try {
      const res = await ServiceRecordAPI.deleteServiceRecord(id);
  ...
  const batchDeleteRecords = useCallback(
    async (ids: string[]): Promise<boolean> => {
      try {
        const res = await ServiceRecordAPI.batchDeleteServiceRecords(ids);

        if (res.success) {
          toast.success(`成功删除 ${ids.length} 条服务记录`);
          return true;
        } else {
