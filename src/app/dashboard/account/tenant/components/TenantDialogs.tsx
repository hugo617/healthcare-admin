...
import { Loader2, AlertTriangle, Trash2, Power, PowerOff, Ban } from 'lucide-react';
...
interface TenantDialogsProps {
  ...
  /** 切换状态回调 */
  onToggleStatus: (tenant: Tenant, action: 'activate' | 'deactivate' | 'suspend') => Promise<boolean>;
  /** 加载状态 */
  loading: boolean;
}
          ...
          <DialogContent className='max-w-md'>
            <DialogHeader>
              <DialogTitle className='flex items-center space-x-2'>
                {action === 'activate' && <Power className='h-5 w-5 text-green-600' />}
                {action === 'deactivate' && <PowerOff className='h-5 w-5 text-yellow-600' />}
                {action === 'suspend' && <Ban className='h-5 w-5 text-red-600' />}
            ...
            <div className='space-y-4'>
              ...
              <div className='rounded-lg border bg-gray-50 p-4'>
                ...
                <div className='mt-2 space-y-1 text-sm text-gray-600'>
                  <div>名称: {tenant.name}</div>
                  <div>代码: {tenant.code}</div>
                  <div>创建时间: {new Date(tenant.createdAt).toLocaleString()}</div>
                </div>
              </div>
              ...
              <div className='flex items-start space-x-2 rounded-lg border-l-4 border-red-400 bg-red-50 p-4'>
                ...
                <div className='text-sm text-red-800'>
                  ...
                  <p className='mt-2 font-medium'>
                    请输入租户代码 "<span className='font-mono bg-red-100 px-1 rounded'>{tenant.code}</span>" 确认删除操作。
                  </p>
                </div>
              </div>
